        ...
        <div className="space-y-4">
          {rankedSignals.map((signal, index) => (
            <Link
              ...
              className="group flex items-center gap-6 p-6 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-[#e63946]/30 transition-all"
            >
              {/* Rank */}
              <div className="flex items-center justify-center w-10 h-10 rounded-full bg-white/10 text-white font-bold text-lg">
                {index + 1}
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="text-lg font-bold text-white group-hover:text-[#e63946] transition-colors truncate">
                    {signal.title}
                  </h3>
                  {signal.isVolatile && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-red-500/20 text-red-400 text-[10px] font-bold uppercase">
              ...
              <div className="text-right">
                <div className="text-2xl font-bold text-white tabular-nums">
                  {signal.currentValue}
                  <span className="text-sm text-white/50 ml-1">{signal.unit}</span>
                </div>
              </div>

              {/* Change */}
              <div className={cn(
                "flex items-center gap-2 min-w-[100px] justify-end",
              ...
              {/* Arrow */}
              <ArrowUpRight className="w-5 h-5 text-white/30 group-hover:text-[#e63946] transition-colors" />
            </Link>
          ))}
        </div>
